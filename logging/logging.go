...
func init() {
	...
	DebugLogger = log.New(os.Stderr, "[ website:DEBUG ] ", log.Ldate|log.Ltime|log.Lshortfile)
	InfoLogger = log.New(os.Stdout, "[ website:INFO  ] ", log.Ldate|log.Ltime)
	WarnLogger = log.New(os.Stderr, "[ website:WARN  ] ", log.Ldate|log.Ltime)
	ErrorLogger = log.New(os.Stderr, "[ website:ERROR ] ", log.Ldate|log.Ltime)

	if os.Getenv("OSC_TESTING") == "true" {
		isTesting = true
	} else {
		isTesting = false
	}
